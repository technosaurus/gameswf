// utility.h	-- tu@tulrich.com Copyright 2001 by Thatcher Ulrich
...
inline int	iclamp(int min, int i, int max) { return imax(min, imin(i, max)); }
inline float	fclamp(float min, float f, float max) { return fmax(min, fmin(f, max)); }

const float LN_2 = 0.693147180559945;
inline float	log2(float f) { return log(f) / LN_2; }

inline int	frnd(float f) { return (int) (f + 0.5); }	// replace with inline asm if desired
