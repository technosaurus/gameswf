...
//			IF_DEBUG(print(stdout));
	...
	struct fill_style
	{
		...
		swf::render::bitmap_info*	m_gradient_bitmap_info;
		...
		swf::render::bitmap_info*	create_gradient_bitmap() const
		...
		// We can use this to set the gradient fill style.
		{
			...
			if (m_type == 0x10)
			{
				...
				swf::render::bitmap_info*	bi = swf::render::create_bitmap_info(im);
				delete im;
		...
		// Push our style parameters into the renderer.
		{
			...
			else if (m_type == 0x10 || m_type == 0x12)
			{
				...
				{
					swf::render::fill_style_bitmap(fill_side, m_gradient_bitmap_info, m_gradient_matrix, swf::render::WRAP_CLAMP);
				}
				else
				{
					// Hack.
