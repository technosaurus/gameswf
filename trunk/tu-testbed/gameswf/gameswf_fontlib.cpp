...
// gameswf::fontlib::set_nominal_glyph_pixel_size()
	...
	struct pending_glyph_info
	{
		...
		pending_glyph_info(font* f, int gi, const glyph& tg)
			:
			m_source_font(f),
			m_glyph_index(gi),
			m_glyph(tg)
	...
	void	finish_current_texture(movie_definition_sub* owner)
	{
		...
		for (int i = 0, n = s_pending_glyphs.size(); i < n; i++)
		{
			...
			pgi->m_glyph.set_bitmap_info(bi.get_ptr());
	//vv		pgi->m_source_font->add_glyph(pgi->m_glyph_index, pgi->m_glyph);
	...
	// characters.
	{
		...
		if (image_hash.get(rgi.m_image_hash, &identical_image))
		{
			...
			if (*(rgi.m_image) == *(identical_image->m_image))
			{
				...
				glyph	identical_tg;
				//vv = identical_image->m_source_font->get_glyph(identical_image->m_glyph_index);
...
//vv					rgi.m_source_font->add_glyph(rgi.m_glyph_index, tg);
				}
				else
				{
	...
	// glyph info into the source fonts.
	//
...
//vv		f->set_glyph_nominal_size(s_glyph_nominal_size);

		for (int i = 0, n = f->get_glyph_count(); i < n; i++)
		{
//vv			if (f->get_glyph(0,i,0).is_renderable() == false)
			if (0)
			{
				shape_character_def*	sh ; //vv= f->get_glyph(0,0,i);
				if (sh)
				{
					rect	glyph_bounds;
					sh->compute_bound(&glyph_bounds);
				...
				//vv		if (render_glyph(&rgi, sh) == true)
						{
...
//vv			font*	f = fonts[i];
	...
	// stream.  This is used by the movie caching code.
	{
		...
		for (int f = 0; f < fonts.size(); f++)
		{
			...
			int	nominal_glyph_size; //vv = fnt->get_glyph_nominal_size();
			...
			for (int g = 0; g < ng; g++)
			{
	//vv			const glyph& tg = fonts[f]->get_glyph(0,0,g);
				const glyph tg; //vv
				...
				if (tg.is_renderable())
				{
					...
					{
//vv						if (tg.m_bitmap_info == owner->get_bitmap_info(bi))
						{
							break;
						}
					}
	...
	// Load a stream containing previously-saved font glyph textures.
	{
		...
		for (int b = 0; b < nb; b++)
		{
			...
			owner->add_bitmap_info(bi.get_ptr());
//vv			assert(bi->get_ref_count() == 2);	// one ref for bi, one for the owner.
		}
...
//vv			fnt->set_glyph_nominal_size(nominal_glyph_size);
			...
			for (int g=0; g<ng; g++)
			{
				...
				{
					//vv fnt->add_glyph(glyph_index, tg);
				}
			}
	...
	void	draw_glyph(const matrix& mat, const glyph& tg, rgba color, int nominal_glyph_height)
